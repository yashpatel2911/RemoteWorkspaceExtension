...
export class SecretStore {
  ...
  private sudoPasswordKey(id: string): string {
    return `remoteWorkspace.sudoPassword.${id}`;
  }
  ...
  getSudoPassword(id: string): Thenable<string | undefined> {
    return this.secrets.get(this.sudoPasswordKey(id));
  }

  setSudoPassword(id: string, value: string): Thenable<void> {
    return this.secrets.store(this.sudoPasswordKey(id), value);
  }

  async clear(id: string): Promise<void> {
    await this.secrets.delete(this.passwordKey(id));
    await this.secrets.delete(this.passphraseKey(id));
    await this.secrets.delete(this.sudoPasswordKey(id));
  }
}
